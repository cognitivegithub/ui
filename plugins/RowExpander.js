...
Ext.extend(Ext.ux.grid.RowExpander, Ext.util.Observable, {
    ...
    getRowClass : function(record, rowIndex, rowParams, ds){
        ...
        if (this.oldGetRowClass) {
          cls += ' '+this.oldGetRowClass(record, rowIndex, rowParams, ds);
        }
        return cls;
    },
...
*/
    ...
    renderer : function(v, p, record){
        p.cellAttr += 'rowspan="2" unselectable="on" ';
        p.css += 'x-unselectable ';
        var noexpand = false;
        if (this.hideExpander) {
          noexpand = this.hideExpander(record);
        }
        return noexpand?'':'<div class="x-grid3-row-expander">&#160;</div>';
    },
   ...
   createExpandingRowPanel: function( record, rowBody, rowIndex ) {
        ...
        if ( !this.expandingRowPanel ) {
            ...
            this.expandingRowPanelItems = [];
        }

        // Add a new panel to the row body if not already there
        if ( this.expandingRowPanel[panelItemIndex] ) {
          if (Ext.get(this.expandingRowPanel[panelItemIndex].id)) {
            Ext.destroy(this.expandingRowPanel[panelItemIndex]);
          } else {
            delete this.expandingRowPanel[panelItemIndex];
          }
        }
            if (!this.expandingRowPanelItems[panelItemIndex] || !this.enableCaching) {
                this.expandingRowPanelItems[panelItemIndex] = this.createExpandingRowPanelItems( this.grid, this.store, record, rowIndex );
            }
            this.expandingRowPanel[panelItemIndex] = new Ext.Panel(
                {
                    // title: 'Custom Fields',
                    layout:'anchor', // this doesn't put the labels there
                    border: false,
                    bodyBorder: false,
                    hideBorders: true,
                    frame: false,
